...
// Wrap your main initialization in a try-catch block
document.addEventListener('DOMContentLoaded', async function() {
    try {
        ...
        // Move these function definitions to the top of the DOMContentLoaded event handler
        ...
        async function scrapeWebsite(url) {
            try {
                const result = await makeAuthenticatedRequest('https://summariser-test-f7ab30f38c51.herokuapp.com/scrape', 'POST', { url });
        ...
        async function getFreeSummariesCount() {
            try {
                const result = await makeAuthenticatedRequest('https://summariser-test-f7ab30f38c51.herokuapp.com/get_free_summaries_count', 'POST', {
                    user_id: await getUserId()
                });
                return result.free_summaries_left !== undefined ? result.free_summaries_left : 0;
            } catch (error) {
        ...
        async function updateModelSelectOptions() {
            ...
            const models = [
                ...
                { value: 'claude-3-5-sonnet-20240620', name: 'Claude', keyName: 'claude-3-5-sonnet-20240620ApiKey' },
        ...
        await updateModelSelectOptions();
        ...
        function loadApiKeysIntoSettingsForm() {
            chrome.storage.local.get(['API_KEY', 'gpt-4o-miniApiKey', 'claude-3-5-sonnet-20240620ApiKey', 'gemini-1.5-flash-8bApiKey', 'mistral-small-latestApiKey'], function(result) {
                ...
                if (apiKeyInput) apiKeyInput.value = result['API_KEY'] || '';
        ...
        async function summarize() {
            ...
            try {
                ...
                const result = await makeAuthenticatedRequest('https://summariser-test-f7ab30f38c51.herokuapp.com/summarize', 'POST', {
                    input: inputText,
                    model: model,
                    user_id: userId,
                ...
                updateFreeSummariesCount(result.free_summaries_left);
            } catch (error) {
        ...
        async function sendUserActivity(userId, modelSelected, searchedFor, searchedData, scrapeData, requestTime) {
            try {
                await makeAuthenticatedRequest('https://summariser-test-f7ab30f38c51.herokuapp.com/user_activity', 'POST', {
                    user_id: userId,
        ...
        async function displayUserInfo() {
            try {
                const userId = await getUserId();
                const data = await makeAuthenticatedRequest('https://summariser-test-f7ab30f38c51.herokuapp.com/get_user_info', 'POST', { user_id: userId });
                if (data.created_at) {
        ...
        // Update the makeAuthenticatedRequest function
        ...
        async function initializeExtension() {
            await login();  // Ensure the user is logged in
            await displayUserInfo();  // Then display user info
            updateModelSelectOptions();  // Update model options
        }
